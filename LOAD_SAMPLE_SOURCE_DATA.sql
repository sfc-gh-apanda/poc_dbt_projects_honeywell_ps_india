...
WITH date_series AS (
    SELECT 
        DATEADD(day, SEQ4(), '2024-01-01'::DATE) as fiscal_date
...
SELECT 
    LPAD(SEQ4(), 10, '0') as CUSTOMER_NUM_SK,
    CASE 
        WHEN SEQ4() <= 40 THEN 'BRP900'
        WHEN SEQ4() <= 75 THEN 'CIP900'
        ELSE 'CIP300'
    ...
    CASE 
        ...
        THEN 'Honeywell ' || CASE UNIFORM(1, 5, RANDOM())
            ...
            ELSE 'Americas Inc'
        END
        ELSE 'Customer ' || SEQ4() || ' ' || CASE UNIFORM(1, 5, RANDOM())
            WHEN 1 THEN 'Inc'
            WHEN 2 THEN 'Ltd'
            WHEN 3 THEN 'Corp'
            WHEN 4 THEN 'GmbH'
