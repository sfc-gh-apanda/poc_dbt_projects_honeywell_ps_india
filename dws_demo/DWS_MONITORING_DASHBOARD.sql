...
USE DATABASE DWS_EDWDEV;

CREATE SCHEMA IF NOT EXISTS DWS_EDWDEV.DWS_MONITORING
...
WHERE database_name = 'DWS_EDWDEV'
...
FROM DWS_EDWDEV.DWS_TRAN.FACT_PORTFOLIO_HOLDINGS
...
FROM DWS_EDWDEV.DWS_TRAN.FACT_TRANSACTIONS
...
SELECT 'FACT_NAV_PRICES', 'Transactional', 'DWS_TRAN', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(created_ts), CURRENT_TIMESTAMP()) <= 24 THEN 'Fresh'
         ...
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDWDEV.DWS_TRAN.FACT_NAV_PRICES
...
SELECT 'DIM_CLIENT', 'Master', 'DWS_MASTER', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Fresh'
         ...
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDWDEV.DWS_MASTER.DIM_CLIENT
...
SELECT 'DIM_ACCOUNT', 'Master', 'DWS_MASTER', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Fresh'
         WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 72 THEN 'Warning'
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDWDEV.DWS_MASTER.DIM_ACCOUNT
...
SELECT 'DIM_FUND', 'Master', 'DWS_MASTER', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Fresh'
         WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 72 THEN 'Warning'
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDWDEV.DWS_MASTER.DIM_FUND
...
FROM DWS_EDWDEV.DWS_REF.DIM_BENCHMARK
...
SELECT 'FACT_FX_RATES', 'Reference', 'DWS_REF', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(created_ts), CURRENT_TIMESTAMP()) <= 24 THEN 'Fresh'
         WHEN DATEDIFF('hour', MAX(created_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Warning'
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDWDEV.DWS_REF.FACT_FX_RATES;
...
FROM (
    ...
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_DIMENSIONS.DIM_CLIENT

    UNION ALL
    SELECT 'DIM_FUND', 'DWS_CLIENT_REPORTING_DIMENSIONS', 'DIMENSION', COUNT(*),
        MAX(dbt_loaded_at)
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_DIMENSIONS.DIM_FUND

    UNION ALL
    SELECT 'DIM_ACCOUNT', 'DWS_CLIENT_REPORTING_DIMENSIONS', 'DIMENSION', COUNT(*),
        MAX(dbt_loaded_at)
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_DIMENSIONS.DIM_ACCOUNT
    ...
    SELECT 'DM_CLIENT_PERFORMANCE', 'DWS_CLIENT_REPORTING_CORE', 'CORE', COUNT(*),
        MAX(dbt_loaded_at)
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_CORE.DM_CLIENT_PERFORMANCE

    UNION ALL
    SELECT 'DM_CASHFLOW_SUMMARY', 'DWS_CLIENT_REPORTING_CORE', 'CORE', COUNT(*),
        MAX(dbt_loaded_at)
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_CORE.DM_CASHFLOW_SUMMARY

    UNION ALL
    SELECT 'FACT_CLIENT_EVENTS', 'DWS_CLIENT_REPORTING_EVENTS', 'EVENTS', COUNT(*),
        MAX(dbt_loaded_at)
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_EVENTS.FACT_CLIENT_EVENTS

    UNION ALL
    SELECT 'AGG_AUM_TIME_SERIES', 'DWS_CLIENT_REPORTING_AGGREGATES', 'AGGREGATE', COUNT(*),
        MAX(dbt_loaded_at)
    FROM DWS_EDWDEV.DWS_CLIENT_REPORTING_AGGREGATES.AGG_AUM_TIME_SERIES
...
GRANT USAGE ON SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_DEVELOPER;
GRANT SELECT ON ALL VIEWS IN SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_DEVELOPER;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_DEVELOPER;

GRANT USAGE ON SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_TESTER;
GRANT SELECT ON ALL VIEWS IN SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_TESTER;

GRANT USAGE ON SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_PROD;
GRANT SELECT ON ALL VIEWS IN SCHEMA DWS_EDWDEV.DWS_MONITORING TO ROLE DWS_PROD;
...
FROM DWS_EDWDEV.INFORMATION_SCHEMA.VIEWS
