...
CREATE SCHEMA IF NOT EXISTS DWS_EDW.DWS_MONITORING
...
FROM DWS_EDW.DWS_TRAN.FACT_PORTFOLIO_HOLDINGS
...
FROM DWS_EDW.DWS_TRAN.FACT_TRANSACTIONS
...
SELECT 'FACT_NAV_PRICES', 'Transactional', 'DWS_TRAN', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(created_ts), CURRENT_TIMESTAMP()) <= 24 THEN 'Fresh'
         ...
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDW.DWS_TRAN.FACT_NAV_PRICES
...
SELECT 'DIM_CLIENT', 'Master', 'DWS_MASTER', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Fresh'
         ...
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDW.DWS_MASTER.DIM_CLIENT
...
SELECT 'DIM_ACCOUNT', 'Master', 'DWS_MASTER', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Fresh'
         WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 72 THEN 'Warning'
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDW.DWS_MASTER.DIM_ACCOUNT
...
SELECT 'DIM_FUND', 'Master', 'DWS_MASTER', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Fresh'
         WHEN DATEDIFF('hour', MAX(update_ts), CURRENT_TIMESTAMP()) <= 72 THEN 'Warning'
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDW.DWS_MASTER.DIM_FUND
...
FROM DWS_EDW.DWS_REF.DIM_BENCHMARK
...
SELECT 'FACT_FX_RATES', 'Reference', 'DWS_REF', COUNT(*),
    ...
    CASE WHEN DATEDIFF('hour', MAX(created_ts), CURRENT_TIMESTAMP()) <= 24 THEN 'Fresh'
         WHEN DATEDIFF('hour', MAX(created_ts), CURRENT_TIMESTAMP()) <= 48 THEN 'Warning'
         ELSE 'Stale' END, CURRENT_TIMESTAMP()
FROM DWS_EDW.DWS_REF.FACT_FX_RATES;
...
GRANT USAGE ON SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_DEVELOPER;
GRANT SELECT ON ALL VIEWS IN SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_DEVELOPER;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_DEVELOPER;

GRANT USAGE ON SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_TESTER;
GRANT SELECT ON ALL VIEWS IN SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_TESTER;

GRANT USAGE ON SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_PROD;
GRANT SELECT ON ALL VIEWS IN SCHEMA DWS_EDW.DWS_MONITORING TO ROLE DWS_PROD;
...
FROM DWS_EDW.INFORMATION_SCHEMA.VIEWS
